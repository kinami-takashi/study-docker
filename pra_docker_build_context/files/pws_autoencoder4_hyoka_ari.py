...
#ネットワーク
class Autoenfilesr(chainer.Chain):
    def __init__(self):
        super(Autoenfilesr, self).__init__(#enfilesr = L.Linear(16, 8),defilesr = L.Linear(8, 16))	#元
                enfilesr = L.Linear(8, 6),defilesr = L.Linear(6, 8))	#使用する特徴量8種類
				#enfilesr = L.Linear(6, 3),defilesr = L.Linear(3, 6))	#卒論でのユニット数：入力層6、中間層3、出力層6

    def __call__(self, x, hidden=False):
        h = F.relu(self.enfilesr(x))
        if hidden:
            #print(h)
            return h
        else:
             #print(self.defilesr(h))
             return self.defilesr(h)
...
ds_test = pd.read_csv('/files/hyouka.csv')	#hyouka.csv:ストレス時,後半少し安静時,5007行1列（1行目のRRIラベル除く）
...
for d in range(1):	#元 range(0,4):
	...
	#hyouka21に合わせるとrange(0,1586):だがhyouka22は1196行の為、エラー発生
		...
		if d == 0:
			ds_test = pd.read_csv('/files/hyouka21.csv')	#テーマ実験？(30分間のRRI)
			ds_test = ds_test['RRI']	#csvファイルからラベルRRI列を取得
			ds_test = ds_test.interpolate(limit_direction='forward', limit_area='inside')	#線形補間
...
dataset = pd.read_csv('/files/dataset.csv')	#/ホーム/1_fitbit_stress内のdataset.csv
...
#モデル作成
model = L.Classifier(Autoenfilesr(), lossfun=F.mean_squared_error)
...
serializers.load_npz('/files/mymodel.npz', model)
...
theme = pd.read_csv('/files/hyouka22.csv')	#テーマ実験(30分間のRRI)
...
plt.savefig("/files/theme_hukugen_gosa_and_rri.png")	#保存
plt.show()										#グラフを表示